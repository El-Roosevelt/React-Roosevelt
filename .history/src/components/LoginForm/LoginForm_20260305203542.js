...
export default function RegisterForm({ onSwitch }) { // Изменил имя на RegisterForm для ясности
  const [formData, setFormData] = useState({
    username: "", // Добавлено поле для БД
    nombre: "",
    apellido: "",
    email: "",
    contrasena: "",
  ...
  const handleSubmitForm = async (e) => {
    ...
    if (Object.keys(newErrors).length === 0) {
      try {
        // Формируем объект точно так, как его ждет сущность Usuarios в Spring Boot
        const userPayload = {
          username: formData.username,
          password: formData.contrasena, // Бэкенд зашифрует его сам
          email: formData.email,
          email_sec: formData.email, // Или добавьте отдельное поле
          nombre: formData.nombre,
          apellido: formData.apellido,
          administrador: false // По умолчанию обычный пользователь
        };

        // ВАЖНО: используем правильный порт 8081 и контекст /backend-rooselvelt
